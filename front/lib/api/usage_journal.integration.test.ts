...
describe("Front Dust usage journal PostgreSQL durability", () => {
  let observer: Client;
  ...
  beforeAll(async () => {
    ...
    if (
      ...
      throw new Error("Refusing a non-test journal database");
    }
  ...
  it("commits attempts before dispatch, preserves frozen outbox bytes, and idempotently delivers", async () => {
    ...
    const operationId = `vertex:${attempt.attemptId}`;
    await markFrontUsageUnknown(attempt.attemptId, operationId);
  ...
  it("retains an unresolved attempt for recovery instead of inventing no-charge", async () => {
    ...
    const operationId = `vertex:${attemptId}`;
    await markFrontUsageUnknown(attemptId, operationId);
    const persisted = await observer.query(
    ...
    expect(persisted.rows[0].providerOperationId).toBe(operationId);
  ...
  it("rejects duplicate provider operations and exact rows without frozen bytes", async () => {
    ...
    const operationId = `same-provider-operation:${randomUUID()}`;
    await markFrontUsageUnknown(first.attemptId, operationId);
    await expect(
      settleFrontUsageExact({
        attempt: first,
    ...
    await expect(
      markFrontUsageUnknown(second.attemptId, operationId)
    ).rejects.toThrow();
    await expect(
      observer.query(
  ...
  it("fences a reclaimed claim even when the worker owner is reused", async () => {
    ...
    const first = (await claimFrontUsageWork("worker-test-reclaim", 100)).find(
  ...
  it("claims fresh exact usage ahead of repeatedly due older work", async () => {
    ...
    const claimed = (await claimFrontUsageWork("worker-test-fairness", 2)).find(
      (claim) => claim.attemptId === fresh.attemptId
    );
    if (!claimed) {
    ...
    expect(claims.some((claim) => oldAttempts.includes(claim.attemptId))).toBe(
      true
    );
  ...
  it("reports tenant-local unresolved work and undelivered exact usage to CRM", async () => {
    ...
    await settleFrontUsageExact({
      attempt: exact,
      providerOperationId: `vertex:${exact.attemptId}`,
      counts: {
        inputTokens: 3,
        outputTokens: 2,
        cacheReadTokens: 0,
    ...
    expect((await readFrontUsageHealth(tenantId)).oldestDeliveryAtSeconds).toBe(
      0
    );
  });
